...
size_t compare_functions(const void *a, const void *b) {
...
void remove_unused_functions(object_t *object) {
	...
	for (i = 0; i < object->areas->length; ++i) {
		...
		for (unsigned int j = 0; j < a->late_immediates->length; ++j) {
			...
			function_metadata_t *meta = get_function_for_address(a, functions, (uint32_t)imm->address);
	...
	for (i = 0; i < functions->length; ++i) {
		...
		if (!func->precious) {
			...
			uint64_t length = func->end_address - func->start_address;
