...
void validate_scas_runtime() {
	...
	if (scas_runtime.output_file == NULL) {
		...
		int i = (int)strlen(out_name);
		while (out_name[--i] != '.' && i != 0);
		if (i == 0) {
			i = (int)strlen(out_name);
		} else {
			out_name[i] = '\0';
		}
		strcat(out_name, ".");
	...
	for (int i = 1; i < argc; ++i) {
		if (argv[i][0] == '-' && argv[i][1] != '\0') {
			if (strcmp("-o", argv[i]) == 0 || strcmp("--output", argv[i]) == 0) {
				...
				size_t l = strlen(scas_runtime.include_path);
...
handle_errors(list_t errors, list_t warnings)
{
	for (unsigned int i = 0; i < errors->length; ++i) {
		...
		if(error->line != NULL){
			...
			if(error->column != 0){
				for(size_t j = error->column; j > 0; j -= 1)
					fputc('.', stderr);
				fputs("^", stderr);
			} else
				fputc('\n', stderr);
	...
	for (unsigned int i = 0; i < warnings->length; ++i) {
		...
		if(warning->line != NULL){
			...
			if(warning->column != 0){
				for (size_t j = warning->column; j > 0; --j)
					fputc('.', stderr);
				fputs("^", stderr);
			}
			free(warning->line);
